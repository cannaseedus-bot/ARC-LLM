...
class GeodesicAttention(nn.Module):
    ...
    def geodesic_distance_sq(x: torch.Tensor, g: torch.Tensor) -> torch.Tensor:
        """Pairwise metric distance^2 for x with shape (B, T, D)."""
        x_i = x.unsqueeze(2)
        x_j = x.unsqueeze(1)
        diff = x_i - x_j
...
class MiniARCLLM(nn.Module):
    """miniARC-LLM v0.1: geodesic-attention micro-transformer."""

    def __init__(self, config: MiniARCConfig):
        super().__init__()
        self.config = config
        self.embed = nn.Embedding(config.vocab_size, config.dim)
        self.layers = nn.ModuleList(
            [MiniArcLayer(config.dim, config.hidden_dim, rank=config.rank) for _ in range(config.depth)]
        )
...
# Backward-compatible aliases from the previous iteration.
ARCConfig = MiniARCConfig
ARCLLM = MiniARCLLM
